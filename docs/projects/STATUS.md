...
|---|---|---|---|---|
| Grade the prop model | not started | Week-1 lines are priced in ff-jarvis `data/props_model.json` (776 lines, model through 2025 wk18, METHODOLOGY §12.29). No test against outcomes yet; the model leans under at the book's lines and its largest edges are news it cannot see | After week-1 box scores post (Tue 2026-09-15): add a `grade` subcommand to `props_model.py` that joins the priced lines to the weekly file and reports hit rate by model bucket. Then, in order: injury report, opponent, role changes. Grade RotoBaller's WR/CB chart too (ff-jarvis `data/consensus/wrcb_wk01.json`, `chart.rows[].score`, 23 WRs): on 2026-09-09 it agreed with the model's REC edge at spearman -0.09, so it is a second opinion, not a feature, until the outcomes say otherwise. Fix `props_client.py` TD rows (`player` is the offer's first participant, the real player is `side`; build.py works around it). DFS lineup separately needs a Yahoo contest CSV export | 2026-09-09 |
